...
class Equipment extends Model
{
    ...
    public function saveJobEquipments($equipments_id, $job_id){

        if( isset($equipments_id) ){
            $equipmentList  =   Equipment::whereIn('uuid', $equipments_id)->get('id');

            if( count($equipmentList) >= 0 ){
                $equipment = new JobEquipment;
                foreach($equipmentList as $equipment_id){

                    $equipment->updateOrCreate(['job_id'=>$job_id,'equipment_id'=>$equipment_id->id],['job_id'=>$job_id,'equipment_id'=>$equipment_id->id]);
                }
            }
        }
    }
